        ...
        while True:
            ...
            if sys.stdin in r:
                ...
                if x == '\t':
                    ...
                    if content:
                        # 如果零时命令列表里面有内容,且用户当前输入了 回车 记录日志

                        # 日志写入数据库
                        data_durable_via_django.make_record_via_mysql(content=content,
...
def run(host, port, username, password, blockhouseuser_id=""):
    ...
    global blockhouseuser_glo_id  # 这里的做法主要是供 POSIX_SHELL() 函数使用
